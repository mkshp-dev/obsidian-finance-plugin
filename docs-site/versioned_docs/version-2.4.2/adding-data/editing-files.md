...
*You can toggle these options in **Settings → Editor**.*
...
*   **Stand-alone Snippets File**: User-defined templates are loaded from a dedicated `snippets.beancount` file stored in your root structured folder layout. When you toggle the feature on in **Settings → Editor**, this file is automatically generated with a `sampleSnippet` template if it is missing.
...
*   **Format on Save**: You can enable this in **Settings → Editor** to format files automatically whenever they are saved.
...
### Linter Modes
Configure how diagnostics are run in **Settings → Editor**:
